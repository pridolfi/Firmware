...
void StartOs_Arch(void)
{
	...
	{
		/* init stack */
		TasksConst[loopi].TaskContext->reg_lr = 0xFFFFFFFD;

		/* init entry point */
		TasksConst[loopi].TaskContext->reg_pc = (uint32) TasksConst[loopi].EntryPoint;

		/* init program status register (only bit T=1)*/
		TasksConst[loopi].TaskContext->reg_xpsr = 1<<24;
	}
